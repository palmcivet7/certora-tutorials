...
rule integrityPoints(address f, address s, address t) {
    ...
    vote(e, f, s, t);
    /// Bug 1
    /// the issue here is because the injected bug allows f and s to be the same address,
    /// points(f) and points(s) both equal the same value + 5
